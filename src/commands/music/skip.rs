...
pub async fn create_skip_response(
    ...
    match handler.queue().current() {
        ...
        None => {
            if tracks_to_skip > 1 {
                create_response_music(&ctx.http, interaction, ParrotMusicMessage::SkipAll).await
            } else {
                create_response_music(&ctx.http, interaction, ParrotMusicMessage::Skip).await
            }
        }
    }
}
